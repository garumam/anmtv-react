...
function FilmesSeries () {
    ...
    useEffect(() => {
        axios.get(`http://localhost:9990/filmes-e-series/${entities.page}`)
        .then((response) =>{
            setEntities({
                isLoading: false,
    ...
    return (
        <Section>
            ...
            <Loading /> 
            :
            <NewsSection isButton title="FILMES E SÉRIES" news={entities.noticias} changePage={changePage} columnLimit={3} />
            }  
        </Section>
    );
}
